...
export function CircuitCall({ contractAddress, connected, onConnectRequired }: CircuitCallProps) {
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      setResult(`Submission accepted. The proof used a private secret code and the public ledger only records the new member count. The private value stays local and is never shown on screen.`);
      setSecretCode('');
      window.localStorage.setItem('anongate-last-secret', normalized);
  ...
  return (
    <section className="card" aria-labelledby="circuit-heading">
      ...
      <p className="muted">The private input is kept in local state only. It never appears in the public ledger view.</p>
      ...
      <div className="button-row">
        <button onClick={() => void handleCall()} disabled={isLoading}>
          {isLoading ? 'Generating proof…' : 'Call circuit'}
        </button>
      </div>
